...
- [x] 全站佈局:頂部資訊列、Header(Logo+搜尋+我要詢價)、分類導覽、Footer
...
- [x] 開發 Supabase 專案連線資料已設定於 `.env.local`;首頁公開讀取已驗證可取得商品/分類/品牌資料。建庫改為一鍵指令 `pnpm db:apply:seed` + `pnpm admin:create`(免開瀏覽器,待補 `SUPABASE_DB_URL` 後在本機執行)
...
- [ ] Cloudflare Turnstile:建 Managed widget(Hostnames 填正式網域與 `*.vercel.app`),Vercel 填 `NEXT_PUBLIC_TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY`(正式環境缺 secret 即 fail-closed)
- [ ] GA4(G-2QE7JNSK8Q):後台關閉加強型評估「依瀏覽器歷程記錄變更網頁瀏覽」(避免 page_view 重複計數);將 `generate_lead` 設為關鍵事件
...
| 日期       | 階段           | 紀錄                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-25 | 工作指引更新 | `AGENTS.md` 開工流程新增收尾規則:每次完成工作回報時,提供可直接用於 git commit 的 title 與 description,方便業主後續提交變更。 |
| 2026-06-25 | 商品詳細頁 loading | 商品詳細頁新增專用骨架屏,版面改為麵包屑、圖庫、商品資訊、價格/詢價操作、規格與相關商品區塊。同步調整 `/products/loading.tsx` 依目前路徑切換列表/詳細頁骨架,避免進入 `/products/[slug]` 時仍短暫看到商品清單骨架。`pnpm` shim 因離線驗簽無法啟動,已改用專案內 `tsc --noEmit`、`eslint .`、`next build` 驗證通過(lint 維持既有 6 個 warning)。 |
| 2026-06-25 | 我要詢價命名與開關 | 前台舊詢價入口對外文案改為「我要詢價」,品項整理區改稱「詢價清單」。新增 `siteConfig.features.quoteRequest.enabled` 集中開關,關閉後 Header/Footer 入口隱藏、商品卡/詳情按鈕停用、`/quote` 顯示暫停受理訊息,`/api/quote` 直接回 503 暫停受理;README 與 docs/01/02/03/04 同步更新。`pnpm` shim 因離線驗簽無法啟動,已改用專案內 `tsc --noEmit`、`eslint .`、`next build` 驗證通過(lint 維持既有 6 個 warning)。 |
| 2026-06-19 | 首頁重要資訊輪播 | 首頁原靜態 Hero 改為 3 則重要資訊輪播,內容涵蓋線上詢價、線材/PCB 備料加工、工業電子材料目錄。新增 `HomeCarousel` client 葉元件,支援 6 秒自動輪播、上一則/下一則、圓點切換、滑鼠與鍵盤焦點暫停、`prefers-reduced-motion`；輪播文案集中於 `siteConfig.homeHighlights`,連結仍由 `routes` 組合。桌機與 375px 實測無水平溢出,手動切換第二則成功,瀏覽器 console 0 error；新增 `scripts/start-local.cmd` 供雙擊啟動 localhost:3000。`pnpm typecheck`、`pnpm lint`、`pnpm build` 通過,lint 維持既有 6 個 warning。 |
| 2026-06-16 | Supabase 本機連線驗證 | `.env.local` 已補正 Supabase Project URL、publishable/anon key 與 server secret/service key；安全檢查只驗證存在與格式,未輸出金鑰內容。以公開 key 連線 Supabase 成功,可讀 active 商品 75 筆、分類 39 筆、品牌 8 筆；本機首頁回應 200 並顯示 Supabase 商品資料,未出現「暫時無法載入」提示。`SUPABASE_DB_URL` 仍可先留空,待需要 db apply/seed 或建立管理者時再補。 |
| 2026-06-16 | 首頁 Supabase env 錯誤處理 | 修正本機缺少 `NEXT_PUBLIC_SUPABASE_URL` 時首頁以 `console.error` 觸發 Next.js 開發錯誤畫面的問題；開發環境改為先檢查 Supabase 公開環境變數,缺設定時不呼叫資料庫查詢、不輸出瀏覽器 console 錯誤,只顯示區塊型「資料暫時無法載入」提示。正式環境仍維持資料或環境錯誤 fail-closed。已驗證 `pnpm typecheck`、`pnpm lint`、`pnpm build` 通過，並以本機測試啟動確認首頁回應 200 且不含 `EnvError`。 |
| 2026-06-16 | Header 品牌文案 | 前台 Header Logo 旁品牌文字改為完整公司名「山強電訊資材有限公司」;副標集中到 `siteConfig.brand.tagline`,文案改為「電子材料、耗材、線材加工、PCB備料加工」。文字區改為 `md` 以上顯示並加上寬度與截斷限制,避免中小尺寸擠壓搜尋框。`pnpm typecheck`、`pnpm lint`、`pnpm build` 通過。 |
| 2026-06-16 | 本機首頁顯示修正 | 首頁改為資料區塊逐一降級:本機缺 Supabase env 或資料庫暫不可用時,Hero、搜尋導流、詢價流程、頁首頁尾仍正常顯示;分類、新品、統計、品牌牆各自顯示「暫時無法載入」提示,不退回示範資料。已驗證 `http://127.0.0.1:3000/` 回 200,包含首頁 Hero 與商品分類區塊,且非整頁錯誤;`pnpm typecheck`、`pnpm lint`、`pnpm build` 通過。 |
| 2026-06-16 | UI/開發錯誤修正 | Header Logo 放大為桌機 80px、行動版 64px,接近「產品目錄」導覽位置且不造成 375px 溢出;移除前台主題切換對 `next-themes` runtime 的依賴,改為站內 theme provider,避免 React/Next 開發模式對 component render 內 `<script>` 的 console error;首頁資料載入改為受控 fail-closed 畫面,缺 Supabase env/DB 不可用時不再觸發 `HomePage` performance measure 連鎖錯誤。`pnpm typecheck`、`pnpm lint`、`pnpm build` 通過;lint 仍保留既有 6 個 React hook warning。 |
| 2026-06-11 | 防濫用+分析    | 我要詢價加 Cloudflare Turnstile(取代原規劃 reCAPTCHA v3→v2:Managed 模式一個 widget 即「隱形檢查、可疑才互動驗證」,免費無上限、無 cookie 隱私疑慮):前端 `ui/turnstile.tsx`(explicit render、跟隨 next-themes 主題、token 過期/錯誤自動清空、送出失敗 reset 一次性 token),`/quote` 表單僅在設定 `NEXT_PUBLIC_TURNSTILE_SITE_KEY` 時渲染;伺服器 `lib/quote/turnstile.ts` siteverify(開發未設 secret 略過;正式缺設定 fail-closed;siteverify 連線失敗回 503 不放行),`/api/quote` 於 Zod 後、RPC 前驗證。另加全站 GA4(G-2QE7JNSK8Q,集中 `site.ts analytics.ga4Id`):`site/google-analytics.tsx` 僅前台+production 載入,page_view 隨路由手動送出(`send_page_view:false`),詢價成功送 `generate_lead`(`lib/analytics.ts`);/admin 不追蹤。.env.example、docs/02、docs/04(Turnstile/GA4 開通步驟+驗收項)同步更新。**待本機 `pnpm typecheck && pnpm lint && pnpm build` 驗證**(沙盒磁碟滿無法執行)。 |
...
| 2026-06-10 | Stage 3        | 我要詢價、/api/quote(原子 RPC+防濫用)、成功頁、Resend 通知與狀態回寫完成。                                                                                                                                                                                                                                                                                                                                                          |
...
| 2026-06-11 | 登入防暴力破解 | `loginAction` 加上與 `/api/quote` 同款滑動視窗 rate limit(每 60 秒 5 次,IP 與 email 雙 key:IP 防掃描、email 防分散式鎖定單一帳號);limiter 由 `lib/quote/rate-limit.ts` 移至共用 `src/lib/rate-limit.ts`(支援 windowMs/maxPerWindow 參數),`/api/quote` import 同步更新。另修復 lint:eslint-config-next 16 已原生 flat config,`eslint.config.mjs` 移除 FlatCompat 改直接 import(原寫法載入即拋 circular structure 錯誤);react-hooks v7 新規則 `set-state-in-effect` 誤抓 6 處 SSR hydration 慣用法,暫降 warn 並註記待重構。沙盒已驗:typecheck ✅、lint ✅(0 errors/6 warnings);build 因沙盒磁碟滿無法執行,**待本機 `pnpm build` 驗證**。 |
