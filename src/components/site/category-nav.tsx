...
export function CategoryNav({ tree }: CategoryNavProps) {
            ...
            <div className="flex-1 overflow-y-auto p-4">
              ...
              {tree.map((cat) => (
                <div key={cat.id} className="mt-1">
                  <Link
                    href={productsUrl({ category: cat.slug })}
                    className="block rounded-md px-2 py-2 text-sm font-semibold text-foreground hover:bg-muted"
                  >
                    {cat.name}
                  </Link>
                  {cat.children.map((child) => (
                    <Link
                      key={child.id}
                      href={productsUrl({ category: child.slug })}
                      className="block rounded-md py-1.5 pl-6 pr-2 text-sm text-muted-foreground hover:bg-muted hover:text-foreground"
                    >
                      {child.name}
                    </Link>
                  ))}
